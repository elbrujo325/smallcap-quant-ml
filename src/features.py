...
class StrategyBuilder:
    ...
    def extract_rules(self, feature_names: List[str]) -> List[StrategyRule]:
        ...
        if hasattr(tree, 'tree_'):
            ...
            value = tree.tree_.value  # Class PROPORTIONS per node (normalized, sklearn>=1.5)
            n_node_samples = tree.tree_.n_node_samples  # Raw sample count per node (siempre real)
        else:
            # Legacy fallback
        ...
        def traverse(node_id, condition_list):
            ...
            if children_left[node_id] == children_right[node_id]:
                ...
                if n_samples >= self.min_samples_leaf:
                    # value[node_id][0] son PROPORCIONES (suman 1.0), no conteos crudos
                    class_props = value[node_id][0]
                    total = class_props.sum()
                    p_label_1 = class_props[1] / total if total > 0 else 0
